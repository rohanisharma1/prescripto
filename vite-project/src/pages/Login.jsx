  ...
  return (
    ...
    <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-sm shadow-lg'>
      ...
      <p>Please {state === 'Sign Up' ? "sign up" : "Login"}  to book appointment</p>
      {
        ...
        <p>Full Name</p>
        <input  className='border border-zinc-300 rounded w-full p-2 nt-1'type = "text" onChange={(e)=> setName(e.target.name)}  value={name}/>
      </div>
      }
      
      <div className='w-full'>
        <p>Email</p>
        <input className='border border-zinc-300 rounded w-full p-2 nt-1' type = "email" onChange={(e)=> setName(e.target.name)}  value={email}/>
      </div>
      <div className='w-full'>
        <p>Password</p>
        <input className='border border-zinc-300 rounded w-full p-2 nt-1' type = "password" onChange={(e)=> setName(e.target.name)}  value={password}/>
      </div>
